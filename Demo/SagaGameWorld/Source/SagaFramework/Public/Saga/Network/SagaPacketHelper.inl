...
[[nodiscard]] TUniquePtr<uint8[]> Serialize() const override;
...
const \
{ \
	return FSagaSerializer::Serializes(myProtocol, mySize, __VA_ARGS__); \
}

#define MAKE_RW_METHODS() \
uint8* Write(uint8* buffer) const override; \
 \
const uint8* Read(const uint8* buffer) override; \

#define IMPL_READ_METHODS(name, ...) \
uint8* \
name::Write(uint8* buffer) \
const \
{ \
	return FSagaSerializer::Serializes(Super::Write(buffer), __VA_ARGS__); \
}

#define IMPL_WRITE_METHODS_V1(name, var1_name) \
const uint8* \
name::Read(const uint8* buffer) \
{ \
	return FSagaSerializer::Deserialize(Super::Read(buffer), var1_name); \
}
...
name::Read(const uint8* buffer) \
{ \
	return FSagaSerializer::Deserialize(FSagaSerializer::Deserialize(Super::Read(buffer), var1_name), var2_name); \
}
...
name::Read(const uint8* buffer) \
{ \
	return FSagaSerializer::Deserialize(FSagaSerializer::Deserialize(FSagaSerializer::Deserialize(Super::Read(buffer), var1_name), var2_name), var3_name); \
}
