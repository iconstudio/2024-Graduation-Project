...
struct [[nodiscard]] name : public saga::FSagaBasicPacket \
{ \
 ...
 \
	[[nodiscard]] std::unique_ptr<std::byte[]> Serialize() const { return saga::Serializes(myProtocol, mySize); } \
}

#define MAKE_SERIALIZE_METHOD(...) \
[[nodiscard]] std::unique_ptr<std::byte[]> Serialize() const \
{ \
...
{ \
 ...
 \
	...
	{} \
 \
	std::byte* Write(std::byte* buffer) const \
	{ \
		return saga::Serialize(Super::Write(buffer), var1_name); \
	} \
 \
	const std::byte* Read(const std::byte* buffer) \
	{ \
...
{ \
 ...
 \
	std::byte* Write(std::byte* buffer) const \
	{ \
		return saga::Serialize(saga::Serialize(Super::Write(buffer), var1_name), var2_name); \
	} \
 \
	const std::byte* Read(const std::byte* buffer) \
	{ \
...
{ \
 ...
 \
	std::byte* Write(std::byte* buffer) const \
	{ \
		return saga::Serialize(saga::Serialize(saga::Serialize(Super::Write(buffer), var1_name), var2_name), var3_name); \
	} \
 \
	const std::byte* Read(const std::byte* buffer) \
	{ \
