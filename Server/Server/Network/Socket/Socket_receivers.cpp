	...
	const noexcept
{
	return Receive(memory).and_then(
		[](unsigned int&&) noexcept -> expected<bool, ErrorCode> {
			...
			[&](net::ErrorCode&& tr_error_code) noexcept -> expected<bool, ErrorCode> {
...
const noexcept
{
	if (Socket::SocketResult sent = Receive(context, memory); not sent)
	{
...
const noexcept
{
	if (Socket::SocketResult sent = Receive(context, memory, size); not sent)
	{
		co_return std::move(sent);
	}
